...
function bindFipeUI(){
  $('#fipeBrandSelect')?.addEventListener('change',handleFipeBrandChange);
  $('#fipeModelSelect')?.addEventListener('change',handleFipeModelChange);
...
async function hydrateFipeFields(data){
  fipeHydrating=true;
  const brandSelect=$('#fipeBrandSelect');
  const modelSelect=$('#fipeModelSelect');
  ...
  try{
    ...
    setSelectOptions(
      brandSelect,
      fipeBrands.map(b=>({value:b.name,label:b.name,code:b.code})),
      'Selecionar marca',
      data.brand||''
    );
    ...
    if(brandCode){
      ...
      setSelectOptions(
        modelSelect,
        fipeModels.map(m=>({value:m.name,label:m.name,code:m.code})),
        'Selecionar modelo',
        data.model||''
      );
      ...
      if(modelCode){
        ...
        const savedYearCode=data.fipeYearCode||'';
        const savedYearValue=data.year||'';
        setSelectOptions(
          yearSelect,
          fipeYears.map(y=>({value:y.name,label:y.name,code:y.code})),
          'Selecionar ano / combustível',
          savedYearValue
        );
        yearSelect.disabled=false;

        let yearCode=savedYearCode;
        if(!yearCode&&savedYearValue)yearCode=fipeYears.find(y=>y.name===savedYearValue)?.code||'';
        setHiddenFipe('fipeYearCode',yearCode);
      }else{
        clearSelect(yearSelect,'Selecione primeiro o modelo');
      }
    }else{
      clearSelect(modelSelect,'Selecione primeiro a marca');
      clearSelect(yearSelect,'Selecione primeiro o modelo');
    }

    // Mantém os valores persistidos sem sobrescrever possíveis ajustes manuais.
    ...
    setFipeStatus('FIPE disponível. Selecione marca, modelo e ano.','success');
  }catch(err){
    console.error('FIPE:',err);

    // Em registros já existentes, preserva os dados salvos para leitura offline.
    if(data.brand){
      setSelectOptions(brandSelect,[], 'Selecionar marca',data.brand);
      brandSelect.disabled=false;
    }
    if(data.model){
      setSelectOptions(modelSelect,[], 'Selecionar modelo',data.model);
      modelSelect.disabled=false;
    }
    if(data.year){
      setSelectOptions(yearSelect,[], 'Selecionar ano / combustível',data.year);
      yearSelect.disabled=false;
    }
    ...
    $('#fipeTransmissionInput').value=data.transmission||'';
    $('#fipeFuelInput').value=data.fuel||'';
    setHiddenFipe('fipeBrandCode',data.fipeBrandCode||'');
    setHiddenFipe('fipeModelCode',data.fipeModelCode||'');
    setHiddenFipe('fipeYearCode',data.fipeYearCode||'');
    setHiddenFipe('fipeCode',data.fipeCode||'');
    setHiddenFipe('fipeReferenceMonth',data.fipeReferenceMonth||'');
    updateFipeReferenceInfo();
...
async function handleFipeBrandChange(){
  if(fipeHydrating)return;
  const brandSelect=$('#fipeBrandSelect');
  const selected=brandSelect.selectedOptions[0];
  const brandCode=selected?.dataset.code||fipeBrands.find(b=>b.name===brandSelect.value)?.code||'';

  setHiddenFipe('fipeBrandCode',brandCode);
  setHiddenFipe('fipeModelCode','');
  clearSelect($('#fipeModelSelect'),'Carregando modelos...');
  clearSelect($('#fipeYearSelect'),'Selecione primeiro o modelo');
  clearFipeDetail();

  if(!brandCode){
    clearSelect($('#fipeModelSelect'),'Selecione primeiro a marca');
    setFipeStatus('Selecione uma marca para continuar.');
    scheduleSave();
    return;
  }
  ...
  try{
    ...
    $('#fipeModelSelect').disabled=false;
    setFipeStatus('Selecione o modelo.','success');
  }catch(err){
    console.error(err);
    setFipeStatus('Não foi possível carregar os modelos.','error');
  }

  scheduleSave();
}

async function handleFipeModelChange(){
  if(fipeHydrating)return;
  const modelSelect=$('#fipeModelSelect');
  const selected=modelSelect.selectedOptions[0];
  const modelCode=selected?.dataset.code||fipeModels.find(m=>m.name===modelSelect.value)?.code||'';

  setHiddenFipe('fipeModelCode',modelCode);
  clearSelect($('#fipeYearSelect'),'Carregando anos...');
  clearFipeDetail();

  // A API usa a descrição do modelo como identificação da versão.
  $('#fipeVersionInput').value=modelSelect.value||'';
  $('#fipeTransmissionInput').value=inferTransmissionFromFipeModel(modelSelect.value);

  if(!modelCode){
    clearSelect($('#fipeYearSelect'),'Selecione primeiro o modelo');
    setFipeStatus('Selecione um modelo para continuar.');
    scheduleSave();
    return;
  }
  ...
  try{
    ...
    console.error(err);
    setFipeStatus('Não foi possível carregar os anos do veículo.','error');
  }
...
async function handleFipeYearChange(){
  ...
  try{
    ...
    $('#fipeVersionInput').value=detail.model||$('#fipeModelSelect').value||'';
    $('#fipeFuelInput').value=detail.fuel||'';
    $('#fipeTransmissionInput').value=inferTransmissionFromFipeModel(detail.model||$('#fipeModelSelect').value);
